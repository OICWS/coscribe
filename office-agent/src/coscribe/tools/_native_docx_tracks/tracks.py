...
Vendored verbatim (bar one import) from `SecurityRonin/docx-mcp`'s
`docx_mcp/document/tracks.py` -- see this directory's NOTICE.md for the
pinned commit and MIT license. Only the `from .base import ...` line was
changed, to `._constants` (a small extract of the three names this file
imports at module level from docx-mcp's own `base.py`; see that module for
the three more host-contract methods -- `_find_para`/`_next_markup_id`/
`_make_run` -- also extracted there) -- coscribe supplies its own
`TracksMixin` host (`_require`/`_find_para`/`_next_markup_id`/`_make_run`/
`_mark`) wrapping an already-open `python-docx` document instead of
docx-mcp's own unzip/parse/repack lifecycle, see `documents.py`'s
`_TrackedDocxHost`.
...
from ._constants import W, _now_iso, _preserve
