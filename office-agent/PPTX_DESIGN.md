...
## 29. `add_pptx_animation` grows from 6 to 203 presets -- the same
"retype/vendor the real registry" upgrade §27 did for transitions,
applied to a much bigger catalog

Continuing the "align with ppt-master" work after §28's image search.
`add_pptx_animation` already leaned on ppt-master's own shipped
`pptx_animation_presets.json` for its 6 existing effects (see the old
code's own comments -- `presetID`/`presetClass`/`presetSubtype` values
and the wrapper-skeleton id-numbering scheme were both cross-checked
against it), but only implemented 6 of that file's real 203 entries,
each one hand-rebuilt as its own `if`/`elif` branch of manual
`etree.SubElement` calls. Checked the file's own metadata before
committing to a plan: `"version": 2"`, `"object_effect_count": 203`,
generated by exporting from **real Microsoft PowerPoint 16** -- not
hand-derived -- across `entrance`/`emphasis`/`exit`/`path` (motion-path)
categories, each with a complete, real `<p:cTn>` XML row (`row_xml`).
Hand-rebuilding the remaining 197 branch-by-branch the way the original
6 were built was not a realistic way to reach the full catalog.

**Vendored the JSON file itself, not retyped** -- `_native_animation_
presets/animation_presets.json` (572KB, commit
`6e3ce9c5a3b994a0e223a14a0f7edd42fd0b9f5`, own `NOTICE.md`), the same
"genuinely opaque data worth diffing against upstream unmodified" tier
§25's vendored formula compiler uses, not §27's "retype a small table"
treatment -- 203 full real XML rows is real, hard-won PowerPoint-export
data, not something worth manually retyping into Python source (and
retyping it would only introduce new typo risk the vendored original
doesn't have). Confirmed hatchling's existing `packages = ["src/
coscribe"]` wheel config already ships non-`.py` data files under `src/
coscribe/` unmodified (the bundled templates' own `.yaml` files are the
existing, working precedent) -- no `pyproject.toml` change needed.

**The mechanism shift, mirroring §27's own "guess the shape" ->
"retype the real table" shift one level further**: rather than
hand-building each preset's XML from scratch, `_instantiate_animation_
preset_row` parses one preset's real `row_xml` at call time and adapts
it in place -- renumbers every `<p:cTn id=...>` sequentially from the
caller's running id counter (`row_xml` was authored against a
hypothetical single-shape slide, always starting at a fixed 5/6/7...
baseline that would collide with a real slide's own ids), retargets
every `<p:spTgt spid="2">` placeholder to the real shape id, and
(`duration_scalable` presets only) scales every finite duration/delay
from the preset's own authored default to the caller's requested
duration. This works identically across all 4 categories including
`path_*` (motion-path) presets -- initially assumed these would need
extra per-shape path-geometry handling, but checked directly: their
`<p:animMotion path="...">` coordinates are already relative/
self-contained in the authored XML (`pathEditMode="relative"`), needing
only the same id/spid substitution as everything else, no special
casing. Confirmed empirically, not assumed: `mso_effect_id`s 83/84/85/150
(`source.media_only_effect_ids` in the JSON, audio/video-only effects)
turned out to already be absent from the 203 "object" entries, so no
extra filtering was needed to keep them out.

**Duration scaling algorithm adapted (not vendored), credited in a code
comment**: `_scale_preset_row_duration`/`_row_effective_duration_ms`/
`_ctn_numeric_delay`, ported from ppt-master's own `pptx_animations.py`
(`_scale_animation_row_duration` and its two small helpers) from
`xml.etree.ElementTree` to this file's own `lxml`. Scales every finite
`dur`/`delay` proportionally except `dur="1"` nodes (the visibility
toggle every entrance/exit preset carries, always kept at exactly 1ms
regardless of ratio -- confirmed this matches real PowerPoint-authored
XML, which never scales this node either), then a correction pass nudges
whichever node actually determines the row's end time so the row's real
length matches the request exactly rather than "close after rounding."
A preset is only duration-scaled when it declares `duration_scalable`
*and* has a real `default_duration_ms` to scale from (8 of 203 don't --
instant appear/disappear toggles and discrete state changes like
changing a shape's font, none of which have a duration control in real
PowerPoint's own UI either) -- for those, the caller's `duration` is
silently ignored and the preset's own authored timing is kept verbatim;
the tool's result now includes `duration_ms` reporting what was actually
used, so this isn't silently invisible to the caller.

**Backward compatibility, verified not just assumed**: the 6 original
short names (`"fade"`, `"fly-in"`, `"exit-fade"`, `"exit-fly"`,
`"emphasis-grow"`, `"emphasis-spin"`) are kept as `_ANIMATION_ALIASES`
onto the real preset key each one turned out to already implement
(matched by presetID/presetSubtype/category triple, not by name
similarity -- e.g. `"emphasis-grow"` maps onto `emphasis_grow_shrink`,
not a same-named `emphasis_grow`, which doesn't exist in the real
catalog). All 22 pre-existing animation
tests pass unchanged against the new generic engine, byte-for-byte same
behavior -- because the tool's existing default `duration=0.5` (500ms)
happens to already equal 4 of the 6 presets' own authored
`default_duration_ms` (500 for fade/fly-in/exit-fade/exit-fly), and for
the other 2 (`emphasis_grow_shrink`/`emphasis_spin`, authored default
2000ms) the preset's single duration-bearing node makes ratio-scaling
and direct-value-setting produce numerically identical output for any
requested duration -- confirmed by working through the math, not assumed
coincidental.

**Generalized the read-back structural verification** the same way:
`_ANIMATION_CHILD_TAGS` (a hand-maintained single-tag-per-preset map,
only covering 6 entries) became `_animation_preset_expected_children`,
deriving the set of a preset's own distinguishing direct-child tags
(`animEffect`/`anim`/`animScale`/`animRot`/`animMotion`/`animClr`/...)
straight from its real `row_xml`, checking that *every* one of them is
present post-save rather than just one hand-picked tag -- correctly
generalizes to presets combining multiple behavior primitives in one row
(e.g. `anim`+`animEffect`+`animScale` together, seen on 4 of the 203).

**New `list_pptx_animation_types()`** mirrors `list_pptx_transition_
types()` exactly (`category="documents"`, `risk_category="READ"`) rather
than dumping 209 names into `add_pptx_animation`'s own docstring. Added
to `test_web.py`'s reviewer-tools contract list (the same real, live
gotcha §20/§27's own list-tool additions already hit).

**Verified real, not just unit-tested**: all 203 presets + 6 aliases (209
total) applied to one real deck and schema-validated + read-back-verified
via the tool's own existing checks -- zero failures, ~8 seconds for the
whole catalog. A 5-animation deck spanning all 4 categories (`entrance_
bounce`, `emphasis_teeter`, `path_bean`, `exit_bounce`, plus a
`by_paragraph` entrance) with mixed on-click/with-previous/after-previous
triggers round-tripped cleanly through python-pptx with warnings treated
as errors and converted through the real LibreOffice `soffice
--convert-to pdf` pipeline with no error -- the same honest ceiling as
§27's own transitions verification: confirming an animation actually
*plays* correctly still needs a human watching real PowerPoint/
LibreOffice, not a static test. New tests: exhaustive schema-validation
across the whole 209-name registry, a new-catalog (non-alias) preset's
structural shape, duration scaling produces the exact requested total
across a multi-node preset, a non-scalable preset correctly ignores the
requested duration and reports its real one, alias and full-name inputs
produce byte-identical XML (id/spid stripped), tool metadata, and the
4-category LibreOffice round-trip above. All 22 pre-existing animation
tests pass unchanged. Full suite, `ruff check`/`mypy` clean.
